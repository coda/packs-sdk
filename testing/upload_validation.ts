...
import type {InlineResourceOutputHint} from '../api_types';
...
import type {MCPToolResourceOutputs} from '../types';
...
import type {ReferenceResourceOutputHint} from '../api_types';
...
function buildMetadataSchema({sdkVersion}: BuildMetadataSchemaArgs): {
  ...
  const defaultAuthenticationValidators: Record<AuthenticationType, z.ZodType<any>> = {
    ...
    [AuthenticationType.OAuth2]: zodCompleteStrictObject<OAuth2DynamicCodeAuthentication>({
      ...
      const {requiresEndpointUrl, endpointKey, authorizationUrl, tokenUrl, resource, useDynamicClientRegistration} =
        authDef;
...
${endpointKey ? 'endpointKey is set' : `requiresEndpointUrl is ${requiresEndpointUrl ?? 'not true'}`}`,
  ...
  const resourceOutputSelectorSchema = z
    ...
    .refine(
      selector =>
        (selector === '$' || /^[A-Za-z_][A-Za-z0-9_]*(?:\[\])?(?:\.[A-Za-z_][A-Za-z0-9_]*(?:\[\])?)*$/.test(selector)) &&
        !selector.split(/[.\[\]]+/).some(segment => ['__proto__', 'prototype', 'constructor'].includes(segment)),
      'Resource output selectors must be property paths with optional [] array segments, or $.',
    );
  const resourceOutputHintSchema = z.discriminatedUnion('representation', [
    zodCompleteStrictObject<ReferenceResourceOutputHint>({
    ...
    zodCompleteStrictObject<InlineResourceOutputHint>({
      selector: resourceOutputSelectorSchema,
    ...
    .superRefine((triggers, context) => {
      ...
      triggers.forEach((trigger, index) => {
        ...
        if (trigger.kind === DefaultTriggerKind.Event) {
          eventTriggers += 1;
          if (eventTriggers > Limits.MaxDefaultEventTriggers) {
            context.addIssue({
              code: 'custom',
              path: [index],
              message: `An agent can only declare ${Limits.MaxDefaultEventTriggers} event default triggers.`,
            });
          }
          return;
        }
  ...
  const mcpToolResourceOutputsSchema = zodCompleteStrictObject<MCPToolResourceOutputs>({
  ...
  const mcpServerSchema = zodCompleteStrictObject<MCPServer>({
    ...
    const tools: MCPToolResourceOutputs[] = server.resourceOutputs || [];
      ...
      .superRefine((data, context) => {
        ...
        function validateSkillTools(skill: {tools?: Tool[]}, basePath: Array<string | number>) {
          (skill.tools || []).forEach((tool, toolIndex) => {
            ...
            if (tool.type === ToolType.Pack && !tool.packId && tool.formulas) {
