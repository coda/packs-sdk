...
export interface TypedSyncFormulaResult<
  ...
  PermissionsContextT extends SyncPassthroughData,
> extends SyncFormulaResult<string, string, any, ContextT, PermissionsContextT> {
  result: T[];
}
...
> extends CommonPackFormulaDef<ParamDefsT>,
  ...
  executeGetPermissions?(
    params: ParamValues<ParamDefsT>,
    request: ExecuteGetPermissionsRequest<K, L, SchemaT, PermissionsContextT>,
...
export type SyncFormula<
  ...
  ContextT extends SyncExecutionContext<any, any>,
  PermissionsContextT extends SyncPassthroughData,
> = SyncFormulaDef<K, L, ParamDefsT, SchemaT, ContextT, PermissionsContextT> & {
...
export function makeNumericFormula<ParamDefsT extends ParamDefs>(
  definition: FormulaOptions<ParamDefsT, PackFormulaDef<ParamDefsT, number>>,
): NumericPackFormula<ParamDefsT> {
...
export function makeStringFormula<ParamDefsT extends ParamDefs>(
  definition: FormulaOptions<ParamDefsT, StringFormulaDefLegacy<ParamDefsT>>,
): StringPackFormula<ParamDefsT> {
  ...
  switch (fullDefinition.resultType) {
    ...
    case ValueType.Boolean: {
      const {
        ...
        resultType: unused,
        ...rest
      } = fullDefinition as FormulaOptions<ParamDefsT, BooleanFormulaDef<ParamDefsT>>;
    ...
    case ValueType.Array: {
      const {
        ...
        items,
        ...rest
      } = fullDefinition as FormulaOptions<ParamDefsT, ArrayFormulaDef<ParamDefsT, SchemaT>>;
    ...
    case ValueType.Object: {
      const {
        ...
        schema,
        ...rest
      } = fullDefinition as FormulaOptions<ParamDefsT, ObjectFormulaDef<ParamDefsT, SchemaT>>;
 ...
 */
export type FormulaDefinitionOptions<
  ParamDefsT extends ParamDefs,
...
export function makeObjectFormula<ParamDefsT extends ParamDefs, SchemaT extends Schema>({
  response,
  ...definition
}: FormulaOptions<ParamDefsT, ObjectResultFormulaDef<ParamDefsT, SchemaT>>): ObjectPackFormula<ParamDefsT, SchemaT> {
...
> {
  ...
  const execute = async function exec(
    params: ParamValues<ParamDefsT>,
    context: ContextT,
  ): Promise<SyncFormulaResult<K, L, SchemaT, ContextT, PermissionsContextT>> {
    let syncResult: SyncFormulaResult<K, L, SchemaDefT, ContextT, PermissionsContextT>;
    try {
    ...
    const returnValue: SyncFormulaResult<K, L, SchemaT, ContextT, PermissionsContextT> = {
      result,
    };
    if (continuation) {
...
>({
  ...
  } as DynamicSyncTableDef<K, L, ParamDefsT, any, ContextT, PermissionsContextT>;
}
...
export function makeTranslateObjectFormula<ParamDefsT extends ParamDefs, ResultT extends Schema>({
  response,
  ...definition
}: FormulaOptions<ParamDefsT, ObjectArrayFormulaDef<ParamDefsT, ResultT>>) {
 ...
 * ```
 */
export function makeEmptyFormula<ParamDefsT extends ParamDefs>(
...
export function maybeRewriteConnectionForFormula<
  ParamDefsT extends ParamDefs,
  T extends FormulaOptions<ParamDefsT, CommonPackFormulaDef<ParamDefsT>> | undefined,
...
/**
 * Name of the text parameter the runtime passes to a suggestion formula.
 * @hidden
 */
export const SUGGESTION_TEXT_PARAMETER_NAME = 'text';

/**
 * Definition of a suggestion formula. {@link makeSuggestionFormula} supplies its parameter, result
 * schema, and purpose.
 * @hidden
 */
export interface SuggestionFormulaDef<ContextT extends ExecutionContext = ExecutionContext> {
  ...
  name: string;
  /** A description of what the formula checks. */
  description: string;
  /** Checks `text`. Each `original` must equal `text.slice(startOffset, endOffset)`. */
  ...
  connectionRequirement?: ConnectionRequirement;
  /** See {@link PackFormulaDef.cacheTtlSecs}. Defaults to 0. */
  cacheTtlSecs?: number;
