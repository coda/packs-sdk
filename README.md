...
In this case, the sync table will be bound to `dynamicUrl` that defines the data
...
dyanmic sync table definition, as described below.
...
- **name**: The name for this category of sync table. The actual name of the table once added to a
  ...
  describing the contents of your pack. For example, if you had a dynamic sync table that sync a sheet
  of data from a Google Sheets spreadsheet, the static name you specify here might be "Sheet",
  indicating that this pack has a building block for syncing data from sheet. You might implement
  you `getName` formula to return the actual name of the sheet, however.
- **getName**: A formula that returns a string, to be used as the title of the table when added to a doc.
  ...
  Google Sheets example, this would call the Google Sheets API with id of the sheet that you're syncing
...
- **getDisplayUrl**: A metadata formula that returns a string, a user-friendly url representing the
  entity being synced. The table in the doc will provide a convenience link to this url so
  the user can easily click through to the source of data in that table.
  In the Google Sheets example, this could be the url to the web-editable spreadsheet, (as opposed to the API url
  of the sheet). The `dynamicUrl` for a sync table is typically an API url for whatever service you're syncing
  from, which is not a meaningful url for an end user. Typically this formula returns a browser-friendly version
...
- **listDynamicUrls**: (Optional) A metadata formula that returns an array of items of the form
  ...
  that require a dynamic url as described above, you must implement this method to list the available
  ...
  In the Google Sheets example, this formula return a list of the sheets that the user has access to;
  the `display` property would be the name of document and the `value` property would be the
  Google Sheets API url for that document. As with all metadata formulas, you will wrap your formula
