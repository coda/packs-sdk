...
export async function handleUpload({
  ...
  if (!packVersion) {
    const nextPackVersionInfo = await client.getNextPackVersion(
      packId,
      {},
      {proposedMetadata: JSON.stringify(manifest)},
    );
    packVersion = nextPackVersionInfo.nextVersion;
    print(`Pack version not provided. Autogenerated one for you: version is ${packVersion}`);
  }
  ...
  try {
    ...
    const metadata = compilePackMetadata({...manifest, version: packVersion});
