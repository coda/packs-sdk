...
.PHONY: do-compile-isolated-vm-24
do-compile-isolated-vm-24:
	rm -rf build-isolated-vm-24
	mkdir build-isolated-vm-24 && \
		cd build-isolated-vm-24 && \
		npm init -y && \
		docker run --rm --platform linux/amd64 -v `pwd`:/var/task public.ecr.aws/sam/build-nodejs24.x:latest \
		  bash -c "microdnf -y install openssl-devel && npm install isolated-vm@${ISOLATED_VM_VERSION}"
	mkdir -p runtime/native/node24/x86_64/isolated-vm/out
	cp build-isolated-vm-24/node_modules/isolated-vm/package.json runtime/native/node24/x86_64/isolated-vm/
	cp build-isolated-vm-24/node_modules/isolated-vm/isolated-vm.js runtime/native/node24/x86_64/isolated-vm/
	cp build-isolated-vm-24/node_modules/isolated-vm/out/isolated_vm.node runtime/native/node24/x86_64/isolated-vm/out/
	rm -rf build-isolated-vm-24

.PHONY: compile-isolated-vm-24
compile-isolated-vm-24:
	if [ ! -f './runtime/native/node24/x86_64/isolated-vm/package.json' ] || \
	   [ `node -p -e "require('./runtime/native/node24/x86_64/isolated-vm/package.json').version"` != $(ISOLATED_VM_VERSION) ]; \
		then $(MAKE) do-compile-isolated-vm-24; \
		else echo "isolated-vm version matches, skipping."; \
	fi
...
view-docs:
	if command -v expect &> /dev/null; then \
		PYTHONPATH=${ROOTDIR} PIPENV_IGNORE_VIRTUALENVS=1 MK_DOCS_SITE_URL=http://localhost:8000/packs-sdk MK_DOCS_GENERATE_ASSETS=false MK_DOCS_FAVICON='assets/favicon-local.svg' expect -c 'set timeout 60; spawn pipenv run mkdocs serve --livereload --watch-theme; expect "Serving on"; exec open "http://localhost:8000"; interact' | grep -v 'mkdocs_site_urls:'; \
	else \
