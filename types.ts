...
/**
 * A dynamically generated suggested action displayed above the chat input.
 * These actions are generated by a skill based on the user's context (e.g., recent meetings, calendar).
 *
 * @hidden In development
 */
...
/**
 * Response format expected from the GenerateSuggestedActions skill.
 *
 * @hidden In development
 */
export interface SuggestedActionsResponse {
  /** Array of suggested actions to display */
  actions: SuggestedAction[];
}

/**
 * Configuration for dynamic suggested actions in the pack's chat interface.
 * When configured, a skill is called to generate contextual action suggestions
 * that appear as buttons above the chat input.
 *
 * @example
 * ```ts
 ...
 * @hidden In development
 */
export interface DynamicSuggestedActionsConfig {
  /**
   * Name of the skill that generates suggested actions.
   * The skill must return JSON in the SuggestedActionsResponse format.
   */
  skillName: string;

  /**
   * Whether to refresh suggestions after each message exchange.
   * Default: false (only refresh on chat open)
   *
   * WARNING: Setting to true will incur LLM costs on every message.
   */
  refreshOnEachMessage?: boolean;
...
export interface PackVersionDefinition {
  ...
  /**
   * Configuration for dynamic suggested actions generated by a skill.
   * When configured, the specified skill is called to generate contextual
   * action suggestions that appear as buttons above the chat input.
   * @hidden
   */
