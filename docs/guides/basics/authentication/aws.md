...
APIs that are part of Amazon Web Services (AWS) use a proprietary authentication scheme known as [AWS Signature Version 4][awsdocs_sv4]. The platform provides built-in support for this type of authentication, and to set it up you only need to write a few lines of simple code. You must specify which AWS service you are connecting to, and each Pack can only connect to a single service.
...
The simplest method of authentication requires that the user provides an access key and secret associated with an AWS user. The platform will use these values to sign outgoing requests. Simply set the authentication type to [`AWSAccessKey`][sdk_awsaccesskey] and specify the AWS service you are connecting to.
...
In addition to supplying the role and external ID, the user authenticating the Pack must also create a trust relationship between the platform's AWS account and their role. This is done by setting a trust policy on the role, associating the role with the account and an external ID.
...
The platform's AWS account has the ID `arn:aws:iam::029208794193:root`, and this is same for all Packs.

The external ID condition is used as a security measure, to ensure that other Packs can't also use the role. When signing a request, the platform will prepend the Pack ID to the user-supplied external ID value, with the format `<PACK_ID>:<EXTERNAL_ID>`. While AWS doesn't strictly require an external ID to use the `AssumeRole` flow, it is recommended for cross-account access scenarios like this and the platform enforces it as a best practice.
