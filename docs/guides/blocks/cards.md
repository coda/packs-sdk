...
If `subtitleProperties` are not defined the card will fall back to using the `featuredProperties` configured on the schema, if any. Featured properties are also used to determine the initial columns to show in a sync table. If you want a schema to include include `featuredProperties` but not display a subtitle, set `subtitleProperties` to an empty array.
