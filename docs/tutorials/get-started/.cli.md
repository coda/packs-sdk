...
So far everything you've built only exists on your local machine, and the platform has no knowledge of it. To see it working in a real doc you'll need to upload your Pack to the platform's servers.


### Register an API token

The `packs` CLI uses the Superhuman Docs API under the hood to upload your code, and likewise needs an API token to operate. Registering an API token is a one-time setup step.
...
1. In the **Superhuman Docs API tokens** section, find the token you just created, and click the **Copy token** link.
...
Now that you have the access configured you can create the new Pack on the platform's servers. This setup step that needs to be done for each Pack you create.

```shell
npx packs create pack.ts --name "Hello World" --description "My first Pack."
```
...
This will create a new, empty Pack on the platform's servers and output its URL in the Pack Studio. It stores the Pack's ID in the new file `.coda-pack.json`.
...
```shell
npx packs upload pack.ts --notes "Initial version."
```
...
1. Run `packs upload` again to upload a new version.

    ```shell
    npx packs upload pack.ts --notes "Changed to Howdy."
    ```
...
<video style="width:auto" loop muted autoplay alt="Recording of adding a dynamic sync table." class="screenshot"><source src="site:images/cli_rebuild.mp4" type="video/mp4"></source></video>
...
[coda_api_token]: https://docs.superhuman.com/account?openDialog=CREATE_API_TOKEN&scopeType=pack#apiSettings
