...
pack.addFormula({
  ...
  execute: async function ([size], context) {
    const resp = await context.fetcher.fetch({method:"GET", url: `https://source.boringavatars.com/beam/${size}`})
    // This API returns direct SVG code used to generate the avatar.
    const svg = resp.body;

    const url = await context.temporaryBlobStorage.storeBlob(Buffer.from(svg), 'image/svg+xml');
    return url;
  },
});
```
