 ...
 */
function makeSyncTable({ name, description, identityName, schema: inputSchema, formula, connectionRequirement, dynamicOptions = {}, tableEntityType, }) {
    ...
    if (tableEntityType === api_types_5.SyncTableEntityType.Users) {
        if (!schemaDef.userEmailProperty) {
            throw new Error(`Sync table schemas with tableEntityType ${api_types_5.SyncTableEntityType.Users} must set a userEmailProperty`);
        }
        if (!schemaDef.userIdProperty) {
            throw new Error(`Sync table schemas with tableEntityType ${api_types_5.SyncTableEntityType.Users} must set a userIdProperty`);
        }
    }
