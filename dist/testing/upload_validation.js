    ...
    if (subError.code === 'invalid_union') {
        ...
        for (const unionIssues of unionErrorGroups) {
            ...
            for (const unionIssue of unionIssues) {
                const isDiscriminantError = unionIssue.code === 'custom' &&
...
const setEndpointPostSetupValidator = zodCompleteObject({
    type: zodDiscriminant(types_16.PostSetupType.SetEndpoint),
    name: z.string(),
...
function buildMetadataSchema({ sdkVersion }) {
    ...
    const defaultAuthenticationValidators = {
        ...
        [types_1.AuthenticationType.OAuth2]: zodCompleteStrictObject({
            ...
            credentialsLocation: z.nativeEnum(types_20.TokenExchangeCredentialsLocation).optional(),
...
${endpointKey ? 'endpointKey is set' : `requiresEndpointUrl is ${requiresEndpointUrl !== null && requiresEndpointUrl !== void 0 ? requiresEndpointUrl : 'not true'}`}`,
    ...
    const variousSupportedAuthenticationValidators = Object.entries(defaultAuthenticationValidators)
        ...
        .map(([_authType, schema]) => schema);
    const reservedAuthenticationNames = Object.values(types_17.ReservedAuthenticationNames).map(value => value.toString());
    ...
    const genericObjectSchema = z.lazy(() => zodCompleteObject({
        ...
        context.addIssue({
            ...
            message: `Too big: expected array to have <=${exports.Limits.FilterableProperties} items`,
        });
    }));
    ...
    const packToolSchema = zodCompleteStrictObject({
        type: z.literal(types_21.ToolType.Pack),
        packId: z.number().optional(),
        formulas: z
            .array(zodCompleteStrictObject({
            formulaName: z
    ...
    const knowledgeToolSourceSchema = z.discriminatedUnion('type', [
        z.object({
            type: z.literal(types_14.KnowledgeToolSourceType.Global),
        }),
        z.object({
            type: z.literal(types_14.KnowledgeToolSourceType.Pack),
            packId: z.number().optional(),
        }),
    ]);
    const knowledgeToolSchema = zodCompleteStrictObject({
        type: z.literal(types_21.ToolType.Knowledge),
        source: knowledgeToolSourceSchema,
    });
    const screenAnnotationSchema = z.discriminatedUnion('type', [
        z.object({
            type: z.literal(types_18.ScreenAnnotationType.Rewrite),
        }),
        z.object({
            type: z.literal(types_18.ScreenAnnotationType.Guide),
        }),
    ]);
    const screenAnnotationToolSchema = zodCompleteStrictObject({
        type: z.literal(types_21.ToolType.ScreenAnnotation),
        annotation: screenAnnotationSchema,
    });
    const embeddedContentSchema = z.discriminatedUnion('type', [
        z.object({
            type: z.literal(types_12.EmbeddedContentType.CopyableBlock),
        }),
        z.object({
            type: z.literal(types_12.EmbeddedContentType.CarouselView),
        }),
        z.object({
            type: z.literal(types_12.EmbeddedContentType.TabView),
        }),
    ]);
    const embeddedContentToolSchema = zodCompleteStrictObject({
        type: z.literal(types_21.ToolType.EmbeddedContent),
        embeddedContent: embeddedContentSchema,
    });
    const mcpToolSchema = zodCompleteStrictObject({
        type: z.literal(types_21.ToolType.MCP),
        serverNames: z.array(z.string()).optional(),
        packId: z.number().optional(),
    });
    const contactResolutionToolSchema = zodCompleteStrictObject({
        type: z.literal(types_21.ToolType.ContactResolution),
    });
    const codaDocsToolSchema = zodCompleteStrictObject({
        type: z.literal(types_21.ToolType.CodaDocsAndTables),
    });
    const mailAndCalendarToolSchema = zodCompleteStrictObject({
        type: z.literal(types_21.ToolType.MailAndCalendar),
    });
    const webSearchToolSchema = zodCompleteStrictObject({
        type: z.literal(types_21.ToolType.WebSearch),
        allowedDomains: z.array(z.string().min(1)).min(1).max(100).optional(),
    });
    const skillModelConfigurationSchema = zodCompleteStrictObject({
        model: z.nativeEnum(types_19.SkillModel),
    ...
    const agentSchema = zodCompleteStrictObject({
        ...
        tools: z
            ...
            tools.forEach((tool, index) => {
                const key = tool.type === types_21.ToolType.Pack ? `${tool.type}:${tool.packId}` : tool.type;
                if (seen.has(key)) {
                    context.addIssue({
                        code: 'custom',
                        path: [index],
                        message: tool.type === types_21.ToolType.Pack
    ...
    const contextRulesSchema = zodCompleteStrictObject({
        signalRegister: z.array(z.nativeEnum(types_6.ContextualTriggerSignalRegister)),
        signalTask: z.array(z.nativeEnum(types_7.ContextualTriggerSignalTask)),
        domain: z.array(domainSchema).max(exports.Limits.MaxBlockedDomains),
        domainCategory: z.array(z.nativeEnum(types_4.ContextualTriggerDomainCategory)),
        domainBlocked: z.boolean().optional(),
    });
    const evaluationConfigSchema = z.discriminatedUnion('strategy', [
        zodCompleteStrictObject({
            strategy: z.literal(types_8.ContextualTriggerStrategy.Regex),
            granularity: z.nativeEnum(types_5.ContextualTriggerGranularity),
            regexPattern: z.string().min(1),
        }),
        zodCompleteStrictObject({
            strategy: z.literal(types_8.ContextualTriggerStrategy.SemanticCentroid),
            granularity: z.nativeEnum(types_5.ContextualTriggerGranularity),
        ...
        zodCompleteStrictObject({
            strategy: z.literal(types_8.ContextualTriggerStrategy.Llm),
            granularity: z.nativeEnum(types_5.ContextualTriggerGranularity),
        }),
    ]);
    const whileWritingTriggerSchema = zodCompleteStrictObject({
        ...
        suggestionColor: z.nativeEnum(types_9.ContextualTriggerSuggestionColor).optional(),
        decorationStyle: z.nativeEnum(types_3.ContextualTriggerDecorationStyle).optional(),
        surfaces: z.array(z.nativeEnum(types_10.ContextualTriggerSurface)).optional(),
    ...
    const defaultTriggerSchema = z.discriminatedUnion('kind', [
        whileWritingTriggerSchema.extend({ kind: z.literal(types_11.DefaultTriggerKind.WhileWriting) }),
    ]);
        ...
        return schema
            ...
            function validateSkillTools(skill, basePath) {
                (skill.tools || []).forEach((tool, toolIndex) => {
                    ...
                    if (tool.type === types_21.ToolType.Pack && !tool.packId && tool.formulas) {
    ...
    const legacyPackMetadataSchema = validateFormulas(unrefinedPackVersionMetadataSchema.extend({
        ...
        category: z.nativeEnum(types_15.PackCategory).optional(),
        logoPath: z.string().optional(),
        exampleImages: z.array(z.string()).optional(),
        exampleVideoIds: z.array(z.string()).optional(),
        minimumFeatureSet: z.nativeEnum(types_13.FeatureSet).optional(),
        ...
        for (const authInfo of getAuthentications(data)) {
            ...
            const readableAuthTitle = name === types_17.ReservedAuthenticationNames.Default ? 'setUserAuthentication()' : `authentication ${name}`;
        ...
        const auth = (_b = data.systemConnectionAuthentication) !== null && _b !== void 0 ? _b : data.defaultAuthentication;
        ...
        const hasSetEndpointStep = postSetup === null || postSetup === void 0 ? void 0 : postSetup.some(step => step.type === types_16.PostSetupType.SetEndpoint);
        ...
        data.mcpServers.forEach((server, i) => {
            ...
            if (!canResolveRelativeUrl) {
                context.addIssue({
                    code: 'custom',
                    path: [`mcpServers[${i}].endpointUrl`],
                    message: "MCP server endpointUrl must be an HTTPS URL unless the pack's authentication provides a connection endpoint (requiresEndpointUrl, endpointKey, or a SetEndpoint postSetup step).",
                });
            }
        });
    })
        ...
        .superRefine((data, context) => {
        const metadata = data;
        const hasMcpToolWithoutPackId = (metadata.skills || []).some(skill => skill.tools.some(tool => tool.type === types_21.ToolType.MCP && !tool.packId));
        ...
        if (!data.agent) {
            if ((data.defaultTriggers || []).length) {
                context.addIssue({
                    code: 'custom',
...
function getAuthentications(data) {
    ...
    if (data.defaultAuthentication) {
        authentications.push({ name: types_17.ReservedAuthenticationNames.Default, authentication: data.defaultAuthentication });
    }
    if (data.systemConnectionAuthentication) {
        authentications.push({
            name: types_17.ReservedAuthenticationNames.System,
