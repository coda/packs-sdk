...
async function handleUpload({ intermediateOutputDirectory, manifestFile, codaApiEndpoint, notes, timerStrategy, }) {
    ...
    if (!packVersion) {
        const nextPackVersionInfo = await client.getNextPackVersion(packId, {}, { proposedMetadata: JSON.stringify(manifest) });
        packVersion = nextPackVersionInfo.nextVersion;
        (0, helpers_5.print)(`Pack version not provided. Autogenerated one for you: version is ${packVersion}`);
    }
    try {
        ...
        const metadata = (0, metadata_1.compilePackMetadata)({ ...manifest, version: packVersion });
