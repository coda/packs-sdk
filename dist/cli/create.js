...
export async function handleCreate({ manifestFile, codaApiEndpoint, name, description, workspace, }) {
    await createPack(manifestFile, codaApiEndpoint, { name, description, workspace });
}
export async function createPack(manifestFile, codaApiEndpoint, { name, description, workspace }) {
    ...
    if (existingPackId) {
        return printAndExit(`This directory has already been registered on ${codaApiEndpoint} with pack id ${existingPackId}.\n` +
