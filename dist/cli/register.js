...
export async function handleRegister({ apiToken, codaApiEndpoint }) {
    const formattedEndpoint = formatEndpoint(codaApiEndpoint);
    if (!apiToken) {
        // TODO: deal with auto-open on devbox setups
        const shouldOpenBrowser = promptForInput('No API token provided. Do you want to visit Coda to create one (y/N)? ', { yesOrNo: true });
