...
describe('compile', () => {
  it('works with source map', async () => {
    const {bundlePath, bundleSourceMapPath} = await compilePackBundle({manifestPath: `${__dirname}/packs/fake.ts`, minify: false});
    try {
  ...
  it('works with minify', async () => {
    const {bundlePath, bundleSourceMapPath} = await compilePackBundle({manifestPath: `${__dirname}/packs/fake.ts`, minify: true});
    try {
      await executeFormulaOrSyncWithVM({
        formulaName: 'Throw',
        params: [],
