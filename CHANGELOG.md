...
- `MCPServer.endpointUrl` now accepts a root-relative path (e.g. `/mcp`) that is resolved against the user's connection endpoint when the pack's authentication sets `requiresEndpointUrl`. Absolute `https` URLs continue to work unchanged.
