...
const monthNamesOnlyRegexpEN =
  '(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)';
const monthNameRegexEN = new RegExp(
  `\\d{1,2}(?:(th|st))?\\s${monthNamesOnlyRegexpEN}|${monthNamesOnlyRegexpEN}\\s\\d{1,2}(?:(th|st))?`,
  'i',
);
 ...
 */
export function parseDate(dateString: string, timezone: string, preferredLocale?: string): Date | null {
 ...
 */
export function shiftToTimezone(date: Date, timezone: string): Date {
  return moment
    .tz(
      [
        date.getFullYear(),
...
export function create(
  ...
  if (hour || minute || second || msec) {
    ...
    m = moment.tz([1900, 0, 1], timezone).year(year).month(month).date(date);
  }
